...
재생을 지원하는 Rust 라이브러리 및 CLI입니다. GStreamer는 필요하지 않습니다.
...
G.711 인코딩과 RTP/RTSP 전송은 Rust로 구현되어 있습니다. FFmpeg는 이 crate에
...
- ONVIF `sendonly` G.711 오디오 백채널을 제공하는 카메라
...
`endpoint_reference`가 있습니다. `DiscoveryOptions::default()`는 자동으로 선택한 로컬
IPv4 주소에서 3초 동안 검색합니다. 여러 NIC 또는 VLAN을 빠짐없이 검색해야 한다면
`interfaces`를 직접 지정합니다.
...
`encoded_bytes`, `packets_sent`, `duration_seconds`가 있습니다.
